...
def run_sarima_forecast(header_id: str, df: pd.DataFrame):
    ...
    _save_results(
        header_id=header_id,
        method_name="SARIMA",
        now_series=pred_mean,
        future_series=pred_future_mean,
...
def run_xgboost_forecast(header_id: str, df: pd.DataFrame):
    ...
    now_series = pd.Series(
        test_supervised["Prediksi_XGBoost"].values,
        index=test_supervised.index,
    )
...
def _save_results(header_id: str, method_name: str,
                   now_series: pd.Series, future_series: pd.Series,
                   mape: float, mae: float, mse: float, rmse: float, r_square: float):
    with Session(engine) as session:
        now_details = [
            ForecastDetail(
            ...
            for idx, val in now_series.items()
        ]
        ...
        future_details = [
            ...
            for idx, val in future_series.items()
        ]

        session.add_all(now_details + future_details)
